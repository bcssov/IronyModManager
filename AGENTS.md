...
- `kill-dotnet.bat` force-terminates every `dotnet.exe` process, while `kill-process.bat` force-terminates running `ironymodmanager.exe` processes. `kill-dotnet.bat` is a manual owner recovery tool, not an agent validation step: agents must never invoke it or substitute a broad process-name kill. Use `kill-process.bat` only when its application process is blocking an intentional operation and its target is confirmed.
...
### Agent .NET build/test process hygiene

Agent-run .NET builds and tests must not leave persistent build, compiler, runner, or test-host processes behind. Prefer non-reused workers where the selected, verified runner supports them; after validation, gracefully clean up resources created by the current run and terminate only confirmed leftovers from that run. Retained workers or severe memory growth make validation failed even when the tests pass. Never blanket-kill `dotnet`, `MSBuild`, `testhost`, `vstest`, or compiler processes by image name: they may belong to the owner, Visual Studio, another terminal, or another agent. Treat cleanup as a `finally` operation for successful, failed, cancelled, interrupted, and retried validation. The runner-specific procedure is in [`docs/TESTING.md`](docs/TESTING.md).
