...
namespace IronyModManager.Services.Registrations
{
    ...
    public class GameRegistration : PostStartup
    {
        ...
        /// </summary>
        private const int HOI4CacheVersion = 22;

        /// <summary>
        /// The stellaris cache version
        /// </summary>
        private const int StellarisCacheVersion = 32;
        ...
        private IGameType GetCK3(string baseUserDir)
        {
            ...
            MapProtonSettings(game, baseUserDir, out baseUserDir);
                        ...
                        try
                        {
                            ...
                            if (File.Exists(exePath))
                            {
                                return new GameSettings
                                {
                                    ...
                                    UserDir = pathResolver.GetUserDirectory(game.LinuxProtonVersion, settings.GameDataPath, game.BaseSteamGameDirectory, game.SteamAppId)
                                };
                            }
                        }
                        catch
        ...
        private void MapGameSettings(IGameType gameType, GameSettings settings)
        {
            if (settings != null && gameType != null)
            {
                ...
                {
                    var oldUserDir = gameType.UserDirectory;
                    gameType.UserDirectory = settings.UserDir;
                    gameType.LogLocation = Path.Combine(gameType.UserDirectory, gameType.LogLocation.Replace(oldUserDir, string.Empty).Trim(Path.DirectorySeparatorChar));
                }
            }
        }

        /// <summary>
        /// Maps the proton settings.
        /// </summary>
        /// <param name="gameType">Type of the game.</param>
        /// <param name="defaultUserDir">The default user dir.</param>
        /// <param name="baseUserDir">The base user dir.</param>
        private void MapProtonSettings(IGameType gameType, string defaultUserDir, out string baseUserDir)
        {
            baseUserDir = defaultUserDir;
            gameType.LinuxProtonVersion = SteamDirectory.GetProtonVersion(gameType.SteamAppId);
            if (!string.IsNullOrWhiteSpace(gameType.LinuxProtonVersion))
            {
                var protonUserDir = pathResolver.GetProtonParadoxUserDir(gameType.BaseSteamGameDirectory, gameType.SteamAppId);
                if (Directory.Exists(protonUserDir))
                {
                    baseUserDir = protonUserDir;
                }
            }
        }
