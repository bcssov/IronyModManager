...
namespace IronyModManager.Services.Tests
{
    ...
    public class GameIndexServiceTests
    {
        ...
        /// </summary>
        [Fact]
        public async Task Should_not_index_definitions_when_definition_exists()
        {
            var storageProvider = new Mock<IStorageProvider>();
            var modParser = new Mock<IModParser>();
            var parserManager = new Mock<IParserManager>();
        ...
        /// </summary>
        [Fact]
        public async Task Should_index_definitions()
        {
            var storageProvider = new Mock<IStorageProvider>();
            var modParser = new Mock<IModParser>();
            var parserManager = new Mock<IParserManager>();
            ...
            gameIndexer.Setup(p => p.WriteVersionAsync(It.IsAny<string>(), It.IsAny<IGame>(), It.IsAny<string>())).Returns((string p1, IGame p2, string p3) => Task.FromResult(false));
        ...
        public async Task Should_not_load_definitions_when_no_game()
        {
            ...
            var service = GetService(gameIndexer, storageProvider, modParser, parserManager, reader, mapper, modWriter, gameService);
            var result = await service.LoadDefinitionsAsync(defs, null, "3.0.3");
        ...
        public async Task Should_not_load_definitions_when_no_version()
        {
            ...
            var service = GetService(gameIndexer, storageProvider, modParser, parserManager, reader, mapper, modWriter, gameService);
            var result = await service.LoadDefinitionsAsync(defs, new Game(), string.Empty);
            result.Should().Be(defs);
        }
        ...
        public async Task Should_not_load_definitions_when_definitions_dont_exist()
        {
            ...
            var service = GetService(gameIndexer, storageProvider, modParser, parserManager, reader, mapper, modWriter, gameService);
            var result = await service.LoadDefinitionsAsync(defs, new Game(), "3.0.3");
            result.Should().Be(defs);
        }
        ...
        public async Task Should_load_definitions()
        {
            ...
            var service = GetService(gameIndexer, storageProvider, modParser, parserManager, reader, mapper, modWriter, gameService);
            var result = await service.LoadDefinitionsAsync(defs, new Game() { Name = "fake game" }, "3.0.3");
